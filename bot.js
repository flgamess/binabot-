...
async function generateAndWaitForImage(type, coin) {
  const prompt = buildImagePrompt(type, coin);
  const seed = Math.floor(Math.random() * 99999);
  const url = `https://image.pollinations.ai/prompt/${prompt}?width=1200&height=630&seed=${seed}&nologo=true&model=flux`;

  // Pollinations necesita tiempo para generar — reintentar hasta 4 veces con espera
  const maxRetries = 4;
  const retryDelay = 8000; // 8 segundos entre intentos

  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      console.log(`Generando imagen (intento ${attempt}/${maxRetries})...`);
      const res = await fetch(url, {
        headers: { "User-Agent": "Mozilla/5.0" },
        // timeout largo porque Pollinations puede tardar
      });
      ...
      if (res.ok) {
        ...
        if (contentType.startsWith("image/")) {
          ...
          return { buffer: await res.buffer(), contentType, url };
        }
      }

      console.log(`Intento ${attempt} fallido (status ${res.status}), esperando ${retryDelay/1000}s...`);
      if (attempt < maxRetries) await sleep(retryDelay);
    } catch (e) {
      console.log(`Intento ${attempt} error: ${e.message}`);
      if (attempt < maxRetries) await sleep(retryDelay);
    }
  }

  throw new Error("No se pudo obtener imagen de Pollinations tras varios intentos");
}

async function uploadImageToBinance(buffer, contentType) {
  // Metodo 1: subir como base64
  try {
    console.log("Subiendo imagen a Binance (base64)...");
    ...
    console.log("Respuesta upload Binance:", JSON.stringify(uploadData).substring(0, 200));

    if (uploadData.code === "000000") {
      const uploadedUrl = uploadData.data?.url || uploadData.data?.imageUrl || uploadData.data;
      if (uploadedUrl && typeof uploadedUrl === "string") {
        console.log("Imagen subida a Binance correctamente");
        return uploadedUrl;
      }
    }
    throw new Error(`Binance upload respondio: ${JSON.stringify(uploadData)}`);
  } catch (e) {
    console.error("Error subiendo a Binance:", e.message);
    return null;
  }
}
...
async function getImageForPost(type, coin) {
  try {
    const { buffer, contentType, url: pollinationsUrl } = await generateAndWaitForImage(type, coin);

    // Intentar subir a Binance
    const binanceUrl = await uploadImageToBinance(buffer, contentType);
    if (binanceUrl) return { uploaded: true, url: binanceUrl };

    // Fallback: URL directa de Pollinations
    console.log("Fallback: usando URL directa de Pollinations");
    return { uploaded: false, url: pollinationsUrl };
  } catch (e) {
    console.error("Error en imagen:", e.message);
    return null;
  }
}
...
async function publishToSquare(content, imageData) {
  // Intentar publicar con imagen primero, luego sin imagen si falla
  const attempts = imageData
    ? [
        { bodyTextOnly: content, imageUrlList: [imageData.url] },   // campo principal
        { bodyTextOnly: content, coverImage: imageData.url },        // campo alternativo
        { bodyTextOnly: content },                                    // sin imagen como ultimo recurso
      ]
    : [{ bodyTextOnly: content }];

  for (let i = 0; i < attempts.length; i++) {
    const body = attempts[i];
    const hasImage = !!body.imageUrlList || !!body.coverImage;
    console.log(`Publicando (intento ${i+1}, imagen: ${hasImage})...`);
    ...
    const data = await response.json();
    console.log("Respuesta Binance Square:", JSON.stringify(data).substring(0, 300));

    if (data.code === "000000") {
      if (hasImage) console.log("Post publicado CON imagen");
      else console.log("Post publicado SIN imagen");
      return data.data?.id || "unknown";
    }

    // Si el error parece relacionado con la imagen, reintentar sin ella
    const errMsg = JSON.stringify(data).toLowerCase();
    const imageError = errMsg.includes("image") || errMsg.includes("url") || errMsg.includes("param");
    if (hasImage && imageError && i < attempts.length - 1) {
      console.log("Error de imagen, reintentando con campo alternativo...");
      continue;
    }

    // Error no relacionado con imagen: fallar
