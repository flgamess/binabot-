 ...
 * Precios reales CoinGecko + Noticias reales CoinDesk + Groq IA
 */
...
const POST_TYPES = [
  "analisis_btc","analisis_eth","analisis_sol","analisis_bnb","analisis_xrp",
  "noticia_crypto","noticia_crypto","noticia_crypto",
  "consejo_trading","resumen_mercado",
  "frase_motivacional","frase_motivacional",
  "comparativa","comparativa",
  "pregunta_btc","pregunta_eth","pregunta_sol","pregunta_bnb","pregunta_xrp",
  "resumen_mercado",
];

// PRECIOS REALES DE COINGECKO (no bloquea servidores externos)
async function getLivePrices() {
  try {
    const url = "https://api.coingecko.com/api/v3/simple/price?ids=bitcoin,ethereum,solana,binancecoin,ripple&vs_currencies=usd&include_24hr_change=true&include_24hr_vol=true&include_high_low=true";
    ...
    const prices = {
      BTC: {
        ...
        high: data.bitcoin?.usd_24h_high?.toFixed(2),
        low: data.bitcoin?.usd_24h_low?.toFixed(2),
      },
      ETH: {
        price: data.ethereum?.usd?.toFixed(2),
        change: data.ethereum?.usd_24h_change?.toFixed(2),
        high: data.ethereum?.usd_24h_high?.toFixed(2),
        low: data.ethereum?.usd_24h_low?.toFixed(2),
      },
      SOL: {
        price: data.solana?.usd?.toFixed(2),
        change: data.solana?.usd_24h_change?.toFixed(2),
        high: data.solana?.usd_24h_high?.toFixed(2),
        low: data.solana?.usd_24h_low?.toFixed(2),
      },
      BNB: {
        price: data.binancecoin?.usd?.toFixed(2),
        change: data.binancecoin?.usd_24h_change?.toFixed(2),
        high: data.binancecoin?.usd_24h_high?.toFixed(2),
        low: data.binancecoin?.usd_24h_low?.toFixed(2),
      },
      XRP: {
        price: data.ripple?.usd?.toFixed(2),
        change: data.ripple?.usd_24h_change?.toFixed(2),
        high: data.ripple?.usd_24h_high?.toFixed(2),
        low: data.ripple?.usd_24h_low?.toFixed(2),
      },
    };

    console.log("Precios OK - BTC: $" + prices.BTC.price + " | ETH: $" + prices.ETH.price + " | SOL: $" + prices.SOL.price);
    return prices;
  } catch (e) {
...
Resumen breve del mercado crypto con estos datos reales:
${pc}
Que sube, que baja, sentimiento general. Maximo 150 palabras.`,

    frase_motivacional: `${base}
Post divertido o motivacional sobre el mundo crypto. Humor sobre HODL, buy the dip, la vida del trader. Muchos emojis. Maximo 100 palabras.`,

    comparativa: `${base}
Compara dos criptomonedas con estos datos reales:
${pc}
...
Ejemplo: "BTC puede llegar a $150,000 este ano?" Desarrolla con contexto breve. Que invite a comentar.`,
